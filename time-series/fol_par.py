  ...
  #  x0_prior = x0_list.mean(axis=0)
    ...
    for pet in pets:
        ...
        for j in range(start_skip):
            cen_f.append( np.sum( (np.exp( (t_vec[t_vec<j]-j) *0.4 )* c_vec[t_vec<j])))
    ...
    np.save('fol_%sclusterNum_%s.npy'%(cluster_num,i), np.array([pet_list,final_list, g,b,g_p,b_p]))
